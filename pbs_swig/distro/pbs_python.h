/*
 * @(#) $Id: pbs_python.h,v 1.1 2001/12/07 19:01:18 sscpbas Exp $
 */
 ...
 *              Made some other improvements so that the python shadow
 *              classe are created with the right fields and automatic
 *              conversion of C-lists and Python-lists.
*/
...
**   Remove the next field so Python does not wrap this field.
*/
struct attrl {
	char	     *name;
	char	     *resource;
...
extern int pbs_alterjob(int connect, char *job_id, 
...
extern char **pbs_selectjob(int connect, struct attropl *IN, char *extend);
...
extern struct batch_status *pbs_statque(int connect, char *id, 
    struct attrl *IN, char *extend); 

extern struct batch_status *pbs_statserver(int connect, struct attrl *IN, 
    char *extend);

extern struct batch_status *pbs_statnode(int connect, char *id, 
    struct attrl *IN, char *extend);

extern char *pbs_submit(int connect, struct attropl *IN, char *script,
